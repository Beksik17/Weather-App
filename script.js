...
const autoUpdateButton = document.querySelector(".auto-update-button");
const currentWeatherDiv = document.querySelector(".current-weather");
const hourlyWeather = document.querySelector(".hourly-weather .weather-list");

const API_KEY = "6e551d27a2bc4fdfa0e100823250911";
...
const setAutoUpdateState = (isLoading, label = "Updated automatically") => {
  if (!autoUpdateButton) return;
  autoUpdateButton.classList.toggle("is-loading", isLoading);
  autoUpdateButton.querySelector(".pill-text").textContent = label;
};
...
const setupWeatherRequest = (cityName) => {
  ...
  return getWeatherDetails(API_URL);
}
...
autoUpdateButton?.addEventListener("click", async () => {
  if (!lastQuery) return;
  setAutoUpdateState(true, "Refreshing...");
  const refreshed = await setupWeatherRequest(lastQuery);
  setAutoUpdateState(false, refreshed ? "Fresh data synced" : "Tap to retry");
});

setupWeatherRequest(lastQuery);